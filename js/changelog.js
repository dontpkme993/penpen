...
const CHANGELOG = [{
	...
	date: '2026-07-27',
	changes: [
		'新增「AI 景深」工具（工具列 AI 群組）：以 Depth Anything V2 估計畫面深度，模型約 47 MB',
		'景深模糊：依深度逐像素改變模糊半徑模擬大光圈，可調對焦深度、模糊強度與景深範圍；對焦面附近保持銳利，離得越遠越模糊',
		'依深度建立選取範圍：拉出深度區間即可選取，適合「只選前景」或「只選遠景天空」，支援邊緣羽化',
		'兩種效果共用同一次深度分析結果，切換操作不需重新推論',
		'分析完成後自動將對焦深度設為畫面中央的深度值：每張圖的深度尺度不同，固定預設值幾乎一定不適用',
		'模糊採用邊緣延伸後再處理，避免瀏覽器的模糊濾鏡把畫布外的透明像素算進來而在四周產生淡出暗角',
	]
}, {
	version: '0.10.1',
	date: '2026-07-26',
	changes: [
		'修正 v0.10.0 的重大問題：Transformers.js v4 只接受 webgpu 與 wasm 兩種推論裝置，v3 的 cpu 已不再有效，導致在取不到 WebGPU 的裝置上 AI 去背、AI 放大、AI 智慧選取三個工具全部無法載入。已改用正確的裝置名稱並實測三者在 CPU 路徑均可正常載入與推論',
		'AI 去背、AI 放大、AI 智慧選取的對話框改為顯示模型的實際下載大小與快取狀態（例：「briaai/RMBG-1.4 · 168 MB · 已快取，無須重新下載」），切換模型或精度時即時更新，資料來自 Transformers.js v4 的 ModelRegistry',
		'AI 去背改為明確指定 fp32 精度，先前未指定時由 Transformers.js 自行挑選，會使顯示的模型大小與實際下載的權重檔不一致',
		'AI 智慧選取新增切換模型後自動重新載入（先前改了模型 ID 仍會沿用舊模型）',
	]
}, {
	version: '0.10.0',
	date: '2026-07-26',
